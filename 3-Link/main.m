...
rng(3686); % start with a particular "seed", for rand() function
...
for n1=1:30
    ...
    if bShowIK
        hold on;
        [th1,th2,th3] = find_ik(pos_source(1),pos_source(2));
        bOK = 0*th1;
        for n=1:length(th1)
            col = collision_avoidance(th1(n),th2(n),th3(n), x_obs, y_obs);
            if ~col
                bOK(n) = 1;
            end
        end
        fi = find(bOK);
        plot3(th1(fi),th2(fi),th3(fi),'k.');
        axis equal;
        axis([-pi pi -pi pi -pi pi])
    end
    disp('RRT 1 begin - initial position to source')
    [states1, ee1] = RRT_new(0, 0, 0, x0_ee, y0_ee, x_obs, y_obs);
    if ~isempty(states1)
        break
    end
    %keyboard
...
for i = 1:1:length(states1.state)
    ...
    xout1(i, 3) = real(states1.state(i).x3);
end
xout1 = flip(xout1);
...
for n2=1:30
    ...
    if bShowIK
        hold on;
        [th1,th2,th3] = find_ik(pos_destination(1),pos_destination(2));
        plot3(th1,th2,th3,'k.');
        axis equal;
        axis([-pi pi -pi pi -pi pi])
    end
    ...
    [states2, ee2] = RRT_new(xout1(end, 1), xout1(end, 2), xout1(end, 3), x1_ee, y1_ee, x_obs, y_obs);
    if ~isempty(states2)
        break
    end
    %keyboard
...
for i = 1:1:length(states2.state)
    ...
    xout2(i, 3) = real(states2.state(i).x3);
end
xout2 = flip(xout2);
